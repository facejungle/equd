...
class theme_supports {
	...
	private static function register_widget_areas() {
		$sidebars = static function () {
			register_sidebar(
				array(
					'name'          => __( 'Archives sidebar', 'equd' ),
					'id'            => 'archive_sidebar',
					'description'   => __( 'Widgets in this area will be shown on all archives.', 'textdomain' ),
	...
	private static function add_image_size() {
		$choose_image_sizes = static function ( $sizes ) {
			return array_merge(
				$sizes,
				array(
					'size_large'  => __( 'Large, max 1920x1080', 'equd' ),
					'size_medium' => __( 'Medium, max 1280x720', 'equd' ),
					'size_small'  => __( 'Small, max 480x480', 'equd' ),
				)
			);
		};
		add_filter( 'image_size_names_choose', $choose_image_sizes );
		$image_sizes = static function () {
			add_image_size( 'size_large', 1920, 1080, array( 'center', 'center' ) );
			add_image_size( 'size_medium', 1280, 720, array( 'center', 'center' ) );
			add_image_size( 'size_small', 480, 480, array( 'center', 'center' ) );
		};
