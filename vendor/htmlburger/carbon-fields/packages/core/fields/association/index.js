    ...
    render()
    {
        ...
        if (! field.duplicates_allowed ) {
            options = produce(
                ...
                draft.map(
                        ( option ) => {
                        option.disabled = !! find(
                                value, ( selectedOption ) => isMatch(
                                selectedOption, {
                                id: option.id,
                                type: option.type,
        ...
        >
         <div className="cf-association__col" ref={ this.selectedList }>
          {
                !! selectedOptions.length && value.map(
                    ( option, index ) => {
...
function aperture( component )
{
    ...
    const combined$ = pipe(
        ...
        map(
            ( combinedActions ) => toProps(
                combinedActions.reduce(
                ( acc, curr ) => ( {
                        ...acc,
...
addFilter(
    'carbon-fields.association.validate', 'carbon-fields/core', ( field, value ) => {
